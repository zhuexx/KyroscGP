...
-- Create Tabs
local keyTab = createTab("Key", UDim2.new(0, 0, 0, 0))
local seedTab = createTab("Seed", UDim2.new(0.25, 0, 0, 0))
local plantTab = createTab("Plant", UDim2.new(0.5, 0, 0, 0))
local eggTab = createTab("Egg", UDim2.new(0.75, 0, 0, 0))
local dupeTab = createTab("Dupe", UDim2.new(1, 0, 0, 0))

-- Adjust tab positions to fit all tabs
local tabWidth = 0.2
local tabPositions = {
    Key = UDim2.new(0, 0, 0, 0),
    Seed = UDim2.new(tabWidth, 0, 0, 0),
    Plant = UDim2.new(tabWidth * 2, 0, 0, 0),
    Egg = UDim2.new(tabWidth * 3, 0, 0, 0),
    Dupe = UDim2.new(tabWidth * 4, 0, 0, 0)
}

for name, pos in pairs(tabPositions) do
    if tabButtons[name] then
        tabButtons[name].Position = pos
        tabButtons[name].Size = UDim2.new(tabWidth, 0, 0.1, 0)
    end
end
...
keyInput.Parent = keyTab

local keyInputCorner = Instance.new("UICorner")
keyInputCorner.CornerRadius = UDim.new(0.2, 0)
...
activateButton.BackgroundColor3 = Color3.fromRGB(0, 100, 150)
activateButton.Text = "ACTIVATE"
activateButton.Font = Enum.Font.SciFi
activateButton.TextColor3 = Color3.fromRGB(200, 255, 255)
activateButton.TextSize = 16
activateButton.Parent = keyTab
...
keyStatusLabel.Size = UDim2.new(0.8, 0, 0.1, 0)
keyStatusLabel.Position = UDim2.new(0.1, 0, 0.6, 0)
keyStatusLabel.BackgroundTransparency = 1
keyStatusLabel.Text = "Status: Not activated"
keyStatusLabel.Font = Enum.Font.SciFi
keyStatusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
keyStatusLabel.TextSize = 14
keyStatusLabel.Parent = keyTab

local hwidLabel = Instance.new("TextLabel")
hwidLabel.Size = UDim2.new(0.8, 0, 0.1, 0)
hwidLabel.Position = UDim2.new(0.1, 0, 0.7, 0)
hwidLabel.BackgroundTransparency = 1
hwidLabel.Text = "HWID: Loading..."
hwidLabel.Font = Enum.Font.SciFi
hwidLabel.TextColor3 = Color3.fromRGB(200, 200, 255)
hwidLabel.TextSize = 12
hwidLabel.TextXAlignment = Enum.TextXAlignment.Left
hwidLabel.Parent = keyTab
...
local HWID = getHWID()
hwidLabel.Text = "HWID: "..HWID:sub(1, 8).."..."..HWID:sub(-4)

-- Key validation function (replace with your actual validation logic)
local function validateKey(key)
    -- This is a placeholder - in a real implementation you would:
    -- 1. Validate against a web API
    -- 2. Or use a secure hash validation system
    -- 3. Or implement a proper licensing system
    
    -- Example simple validation (not secure - replace with your own logic)
    local validKeys = {
        ["ABC123-XYZ456"] = true,
        ["TEST-KEY-2023"] = true,
...
-- Save/Load functions using DataStores (optional)
local function saveActivation()
    -- In a real implementation, you might save to a DataStore
    -- or use some other persistence method
end

local function loadActivation()
    -- In a real implementation, you might load from a DataStore
    -- or check a saved file
    return false
end

-- Initial load
isActivated = loadActivation()
if isActivated then
    keyStatusLabel.Text = "Status: Activated (Persistent)"
...
activateButton.MouseButton1Click:Connect(function()
    if isActivated then
        ...
        isActivated = false
        activationTime = 0
        keyStatusLabel.Text = "Status: Deactivated"
        keyStatusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
        keyInput.Text = ""
        activateButton.Text = "ACTIVATE"
        saveActivation()
        ...
        if key == "" then
            keyStatusLabel.Text = "Error: Please enter a key"
        ...
        if validateKey(key) then
            ...
            keyStatusLabel.Text = "Status: Activated"
            keyStatusLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
            keyInput.Text = "************"
            activateButton.Text = "DEACTIVATE"
            saveActivation()
            ...
            sound.SoundId = "rbxassetid://9045567253" -- Success sound
            sound.Volume = 0.5
            sound.Parent = gui
            sound:Play()
            game:GetService("Debris"):AddItem(sound, 2)
        else
            keyStatusLabel.Text = "Error: Invalid key"
            task.delay(2, function()
                if not isActivated then
                    keyStatusLabel.Text = "Status: Not activated"
                end
            ...
            -- Play an error sound
            local sound = Instance.new("Sound")
            sound.SoundId = "rbxassetid://9045566887" -- Error sound
            sound.Volume = 0.5
            sound.Parent = gui
            sound:Play()
            game:GetService("Debris"):AddItem(sound, 2)
...
-- Authorization check for all features
local function checkAuthorization()
    if not isActivated then
        -- Switch to key tab and show message
        for _, frame in pairs(tabFrames) do frame.Visible = false end
        keyTab.Visible = true
        for _, button in pairs(tabButtons) do button.BackgroundColor3 = Color3.fromRGB(40, 40, 60) end
        if tabButtons["Key"] then
            tabButtons["Key"].BackgroundColor3 = Color3.fromRGB(60, 60, 80)
        end
        
        keyStatusLabel.Text = "Please activate first!"
        keyStatusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
        return false
    end
    return true
...
local function duplicatePet()
    if not checkAuthorization() then return false end
